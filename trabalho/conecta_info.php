	...
	<?php
	 include("conexão.php");
	    if (!empty($_POST['insere'])) {
		   ...
		   $rua = $_POST['rua'];           
           $data=$_POST["data"];
				$stmt = $conn->prepare("INSERT INTO assalto(bairro,rua,data) VALUES (:BAIRRO,:RUA,:DATA)");
						$stmt->bindParam(':RUA', $rua);	
						 $stmt-> bindParam(':BAIRRO',$bairro);
						 $stmt-> bindParam(':DATA',$data);
			  $stmt-> execute();
				if($stmt==true){
					echo 'As informações sobre o seu assalto foram salvos<br>Obrigado por sua Colaboração'; 
				}
        }	   
	?>
