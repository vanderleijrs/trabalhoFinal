...
<head>
	...
	<?php
	    include("conexão.php");
	    if (!empty($_POST['insere'])) {
           $bairro = $_POST['bairro'];
		   $rua = $_POST['rua'];           
           $prob=$_POST["prob"];			  
				$stmt = $conn->prepare("INSERT INTO ruas(bairro,rua,problema) VALUES (:BAIRRO,:RUA,:PROB)");
						$stmt->bindParam(':RUA', $rua);	
						 $stmt-> bindParam(':BAIRRO',$bairro);
						 $stmt-> bindParam(':PROB',$prob);
			  $stmt-> execute();
				if($stmt==true){ echo 'As informações sobre o sua rua foram salvos<br>Obrigado por sua colaboração'; }
